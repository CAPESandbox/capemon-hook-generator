...
def obtain_winapi_file():
    if not os.path.exists("winapi_categories.json"):
        print("[*] winapi_categories.json file not detected. Downloading from repo (https://github.com/reverseame/winapi-categories)\n")
        r = requests.get("https://raw.githubusercontent.com/reverseame/winapi-categories/refs/heads/main/winapi_functions_by_category.json")
...
def create_hook_h():
    if not os.path.exists("extended_hooks.h"):
        with open("extended_hooks.h", "w") as file:
            file.write("/*\n\tFile autogenerated by capemon hook generator (https://github.com/reverseame/cape-hook-generator)\n")
